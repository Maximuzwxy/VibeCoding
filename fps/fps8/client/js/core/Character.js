class Character {
    ...
    createLabel(text, color, size = { width: 128, height: 64 }) {
        ...
        context.font = `bold ${size.height * 0.4}px Arial`;
        const textWidth = context.measureText(text).width;
        const actualWidth = Math.max(textWidth + 20, size.width);
        
        canvas.width = actualWidth;
        
        context.fillStyle = color;
        context.fillRect(0, 0, actualWidth, size.height);
        
        context.fillStyle = '#ffffff';
        context.font = `bold ${size.height * 0.4}px Arial`;
        context.textAlign = 'center';
        context.textBaseline = 'middle';
        context.fillText(text, actualWidth / 2, size.height / 2);
        
        const texture = new THREE.CanvasTexture(canvas);
        const material = new THREE.SpriteMaterial({ map: texture });
        const sprite = new THREE.Sprite(material);
        sprite.scale.set(actualWidth / 100, size.height / 100, 1);
    ...
    triggerExplosion(position) {
        ...
        for (const character of Character.allCharacters) {
            ...
            const shooterPos = this.skeleton.root.position.clone();
            shooterPos.y = 0;
                ...
                if (charIntersects.length === 0) {
                    if (distToExplosion <= innerRadius) {
                        ...
                        if (this.onGrenadeHit) {
                            const targetId = character.npcId || character.id;
                            this.onGrenadeHit(shooterPos.clone(), charPos.clone(), targetId, 2);
                        }
                    } else {
                        character.takeDamage(1);
                        if (this.onGrenadeHit) {
                            const targetId = character.npcId || character.id;
                            this.onGrenadeHit(shooterPos.clone(), charPos.clone(), targetId, 1);
                        }
                    }
                }
            }
