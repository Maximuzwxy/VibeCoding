...
为保持代码清晰，建议将 HTML、CSS、JavaScript 分离到不同文件：
...
- `index.html`：包含 HTML 结构和 `importmap` 配置
- `style.css`：包含所有样式
- `main.js`：包含 Three.js 场景、渲染器、相机、控制逻辑等所有 JavaScript 代码
...
#### 脚本引入
- 使用传统 `<script>` 标签引入 Three.js（r128 版本）
- CDN 地址：`https://cdnjs.cloudflare.com/ajax/libs/three.js/r128/three.min.js`
- OrbitControls：`https://cdn.jsdelivr.net/npm/three@0.128.0/examples/js/controls/OrbitControls.js`
- 原因：r128 版本保留了传统的 `examples/js` 文件夹，可以直接用 `file://` 协议打开，无需 HTTP 服务器
...
#### 立方体 (Cube)
- 尺寸：15 x 15 x 15 单位
- 材质：`MeshBasicMaterial`，透明设置 `transparent: true, opacity: 0.15`
- 渲染属性：`side: THREE.DoubleSide`（可看到背面）、`depthWrite: false`（避免透明物体深度冲突）
- 边缘：使用 `LineSegments` 和 `EdgesGeometry` 绘制可见的 12 条边
- 边缘颜色：青色（0x48dbfb）
- 边缘透明度：0.6
- 边缘渲染顺序：`renderOrder = 1`（确保边缘在立方体面之上渲染）
