class NPCAI {
    ...
    findNearestTarget() {
        ...
        for (const potentialTarget of this.npc.players) {
            if (potentialTarget.isDead) continue;
            if (potentialTarget.team === this.npc.team) continue;

            const dist = npcPos.distanceTo(potentialTarget.skeleton.root.position);
            if (dist < nearestDist) {
                nearestDist = dist;
    ...
    shootAtTarget(distance) {
        ...
        let baseHitRate;
        if (distance < 10) {
            baseHitRate = this.hitRate.closeRange;
        } else if (distance < 25) {
            baseHitRate = this.hitRate.midRange;
        } else {
            baseHitRate = this.hitRate.farRange;
        }

        let finalHitRate = baseHitRate;
        if (this.target.isMoving) {
            finalHitRate *= this.hitRate.targetMovePenalty;
        }
        if (this.npc.isMoving) {
            finalHitRate *= this.hitRate.npcMovePenalty;
        }
    ...
    handleInCoverMoving(deltaTime, distToTarget) {
        ...
        if (this.movingTargetPos) {
            ...
            if (distToMovingTarget > 1.0) {
                this.npc.avoidObstacle(this.movingTargetPos, deltaTime, CHARACTER_CONFIG.moveSpeed * 0.6, this.target.skeleton.root.position);
            } else {
                this.movingTargetPos = this.calculateCoverEdgePosition(this.movingDirection);
            }
        }
