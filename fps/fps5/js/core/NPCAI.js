class NPCAI {
    ...
    updateDebugMarker(position) {
        if (!this.debugMarker) {
            const geometry = new THREE.SphereGeometry(0.3, 8, 8);
            const material = new THREE.MeshBasicMaterial({ 
                color: this.npc.aggressionMode === 'aggressive' ? 0xff0000 : 0x0000ff 
            });
            this.debugMarker = new THREE.Mesh(geometry, material);
            this.npc.scene.add(this.debugMarker);
        }

        if (position) {
            this.debugMarker.position.set(position.x, 2, position.z);
    ...
    ensureCoversInitialized() {
        if (this.covers.length === 0) {
            this.initCovers();
            console.log(`[NPCAI] 掩体初始化完成，找到 ${this.covers.length} 个掩体`);
        }
    }
    ...
    findBestCover() {
        ...
        const npcPos = this.npc.skeleton.root.position;
        const targetPos = this.target.skeleton.root.position;
    ...
    handleStuckDetection(deltaTime, targetPos) {
        const currentPos = this.npc.skeleton.root.position.clone();
        ...
        if (this.isSliding) {
            ...
            if (this.slideTarget) {
                ...
                this.npc.setAimTarget(this.target.skeleton.root.position);
            }
            return true;
        }
    ...
    update(deltaTime) {
        ...
        const distToTarget = this.npc.skeleton.root.position.distanceTo(this.target.skeleton.root.position);
        ...
        switch (this.state) {
            ...
            case 'IN_COVER':
                this.handleInCover(deltaTime, distToTarget);
                break;
    ...
    handleNavigating(deltaTime, distToTarget) {
        ...
        if (!this.currentCover) {
            ...
            if (this.currentCover) {
                ...
                this.updateDebugMarker(this.coverPos);
            }
        }

        if (!this.currentCover) {
            this.npc.chaseTowards(this.target.skeleton.root.position, deltaTime);
            this.npc.setAimTarget(this.target.skeleton.root.position);
            return;
        }
        ...
        if (distToTargetPos > 2.5) {
            this.npc.avoidObstacle(targetPos, deltaTime, CHARACTER_CONFIG.moveSpeed, this.target.skeleton.root.position);
            this.npc.setAimTarget(this.target.skeleton.root.position);
    ...
    handleInCover(deltaTime, distToTarget) {
    ...
    handleEngaging(deltaTime, distToTarget) {
        ...
        if (this.strafeTimer <= 0) {
            this.strafeDirection = this.selectStrafeDirection();
            this.strafeTimer = this.npc.aggressionMode === 'aggressive' ? 
                (3 + Math.random() * 2) : 
                (2 + Math.random());
        }

        this.npc.strafeAround(this.target.skeleton.root.position, deltaTime, this.strafeDirection);
        this.npc.setAimTarget(this.target.skeleton.root.position);
    ...
    handleRetreating(deltaTime, distToTarget) {
        ...
        const toTargetDir = new THREE.Vector3().subVectors(
            this.target.skeleton.root.position,
            this.npc.skeleton.root.position
        ).normalize();
